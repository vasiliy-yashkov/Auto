/******************************************************************************/
/***         Generated by IBExpert 2017.12.13.1 15.05.2018 9:55:18          ***/
/******************************************************************************/
...
SET CLIENTLIB 'E:\core\Firebird-3.0.2.32703-0_Win32\fbclient.dll';
...
CREATE TABLE "AUTO" (
    AUTO_ID          BIGINT GENERATED BY DEFAULT AS IDENTITY,
    AUTO_COLOR       VARCHAR(250) NOT NULL,
    STATUS_ID        BIGINT NOT NULL,
    MODIFICATION_ID  BIGINT,
    AUTO_FULL        COMPUTED BY ((select * from MODEL_DETAIL(MODEL_ID)) || ' Р¦РІРµС‚: ' || AUTO_COLOR ||
' Р”РІРёРіР°С‚РµР»СЊ: ' || (select * from ENGINE_DETAIL(ENGINE_ID)) ||
' РњРѕРґ.: ' || (select * from MOD_DETAIL(MODIFICATION_ID))),
...
CREATE TABLE PERSON (
    ...
    PERSON_FULL         COMPUTED BY (( PERSON_LAST_NAME || ' ' || PERSON_NAME || ' ' || PERSON_MIDDLE_NAME ))
);
...
CREATE TABLE EMPLOYEE (
    ...
    EMPLOYEE_FULL        COMPUTED BY ((select first 1 p.PERSON_FULL from PERSON p where p.PERSON_ID = PERSON_ID) ||
', ' || (select first 1 pt.POST_NAME from POST pt where pt.POST_ID = POST_ID))
);

CREATE TABLE ENGINE (
    ENGINE_ID           BIGINT GENERATED BY DEFAULT AS IDENTITY,
    ENGINE_VOLUME       NUMERIC(15,2) NOT NULL,
    ENGINE_POWER        INTEGER NOT NULL,
    ENGINE_TYPE         VARCHAR(250) NOT NULL,
    ENGINE_CONSUMPTION  NUMERIC(15,2) NOT NULL,
    ENGINE_FULL         COMPUTED BY (('РћР±СЉРµРј: ' || ENGINE_VOLUME || 'Р», РњРѕС‰РЅРѕСЃС‚СЊ: ' || ENGINE_POWER || 'Р».СЃ., РўРёРї: ' ||
ENGINE_TYPE || ', Р Р°СЃС…РѕРґ Р»(100РєРј): ' || ENGINE_CONSUMPTION))
);

CREATE TABLE MARK (
    MARK_ID    BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE MODEL (
    ...
    MODEL_FULL  COMPUTED BY ((select * from MARK_DETAIL(MARK_ID)) || ' ' || MODEL_NAME)
);

CREATE TABLE MODIFICATION (
    MODIFICATION_ID    BIGINT GENERATED BY DEFAULT AS IDENTITY,
    MODIFICATION_NAME  VARCHAR(250) DEFAULT ' ',
    MODIFICATION_BODY  VARCHAR(250) NOT NULL,
    TRANSMISSION       VARCHAR(100) NOT NULL,
    "FULL"             COMPUTED BY ((MODIFICATION_NAME || ' РљСѓР·РѕРІ: ' || MODIFICATION_BODY || ' РљРџРџ: ' ||
TRANSMISSION))
);

CREATE TABLE PAY (
    PAY_ID    BIGINT GENERATED BY DEFAULT AS IDENTITY,
    PAY_NAME  VARCHAR(250) NOT NULL
);

CREATE TABLE PERSON_INFO (
    PERSON_INFO_ID  BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
ALTER TABLE "AUTO" ALTER AUTO_ID RESTART WITH 3;
ALTER TABLE CLIENT ALTER CLIENT_ID RESTART WITH 0;
ALTER TABLE DELIVERY ALTER DELIVERY_ID RESTART WITH 0;
ALTER TABLE EMPLOYEE ALTER EMPLOYEE_ID RESTART WITH 0;
ALTER TABLE ENGINE ALTER ENGINE_ID RESTART WITH 2;
ALTER TABLE MARK ALTER MARK_ID RESTART WITH 2;
ALTER TABLE MODEL ALTER MODEL_ID RESTART WITH 5;
ALTER TABLE MODIFICATION ALTER MODIFICATION_ID RESTART WITH 0;
ALTER TABLE PAY ALTER PAY_ID RESTART WITH 0;
ALTER TABLE PERSON ALTER PERSON_ID RESTART WITH 0;
ALTER TABLE PERSON_INFO ALTER PERSON_INFO_ID RESTART WITH 0;
ALTER TABLE POST ALTER POST_ID RESTART WITH 0;
ALTER TABLE PROVIDER ALTER PROVIDER_ID RESTART WITH 0;
ALTER TABLE SALE ALTER SALE_ID RESTART WITH 31;
ALTER TABLE STATUS ALTER STATUS_ID RESTART WITH 0;
